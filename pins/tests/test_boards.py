...
from pins.boards import BaseBoard, BoardManual, BoardRsConnect
...
class TestBoard:
    def test_validate_pin_name_root(self, board):
    ...
    class TestPinWrite:
        ...
        def test_prepare_pin(self, board, tmp_path: Path):
            df = pd.DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]})
        ...
        def test_pin_write_type_error(self, board):
            class C:
                pass
        ...
        def test_feather_deprecated(self, board):
            df = pd.DataFrame({"x": [1, 2, 3]})

            with pytest.warns(DeprecationWarning):
                board.pin_write(df, "cool_pin", type="feather")

        def test_write_file_raises_error(self, board, tmp_path):
            df = pd.DataFrame({"x": [1, 2, 3]})
        ...
        def test_rsc_index_html(self, board, tmp_path: Path, snapshot):
    ...
    class TestPinUploadAndDownload:
        def test_roundtrip(self, board_with_cache, tmp_path):
        ...
        def test_filename_many_suffixes(self, board_with_cache, tmp_path):
            # create and save data
            df = pd.DataFrame({"x": [1, 2, 3]})
        ...
        def test_filename_no_suffixes(self, board_with_cache, tmp_path):
            # create and save data
            df = pd.DataFrame({"x": [1, 2, 3]})
        ...
        def test_filename(self, board_with_cache, tmp_path):
            # create and save data
            df = pd.DataFrame({"x": [1, 2, 3]})
        ...
        def test_no_cache_error(self, board, tmp_path):
            df = pd.DataFrame({"x": [1, 2, 3]})
            path = tmp_path / "data.csv"
            df.to_csv(path, index=False)
        ...
        def test_path_list(self, board_with_cache, tmp_path):
            # create and save data
            df = pd.DataFrame({"x": [1, 2, 3]})
    ...
    class TestPinReadWriteDifferentTypes:
        @parametrize(
            "obj, type_",
            [
                (df, "csv"),
        ...
        def test_write_type(self, board, obj, type_, request):
        ...
        def test_read_insecure_fail_default(self, board):
        ...
        def test_read_insecure_fail_board_flag(self, board):
        ...
        def test_read_insecure_succeed_board_flag(self, board):
            # board flag prioritized over env var
            with rm_env(PINS_ENV_INSECURE_READ):
    ...
    class TestMixedVersionedUnversioned:
        @pytest.mark.parametrize("versioned", [None, False])
        def test_board_unversioned_pin_write_unversioned(
            self, versioned, board_unversioned
        ):
            board_unversioned.pin_write(
                {"a": 1}, "test_pin", type="json", versioned=versioned
            )
            board_unversioned.pin_write(
                {"a": 2}, "test_pin", type="json", versioned=versioned
            )

            assert len(board_unversioned.pin_versions("test_pin")) == 1
            assert board_unversioned.pin_read("test_pin") == {"a": 2}

        def test_board_unversioned_pin_write_versioned(self, board_unversioned):
            board_unversioned.pin_write(
                {"a": 1}, "test_pin", type="json", versioned=False
            )
            board_unversioned.pin_write({"a": 2}, "test_pin", type="json", versioned=True)

            assert len(board_unversioned.pin_versions("test_pin")) == 2

        def test_board_versioned_pin_write_unversioned(self, board):
    ...
    @pytest.fixture
    def pin_del(self, board, df, pin_name):
    ...
    @pytest.fixture
    def pin_prune(self, board, df, pin_name):
        today = datetime.now()
        ...
        board.pin_write(
            df, pin_name, type="csv", title="some title", created=two_days_ago
        )
    ...
    class TestPinDelete:
        def test_pin_delete(self, board, df, pin_name, pin_del):
            board.pin_delete(pin_name)

            assert board.pin_exists(pin_name) is False

    class TestPinDeleteVersion:
        def test_older(self, board, pin_name, pin_del):
        ...
        def test_latest(self, board, pin_name, pin_del):
            meta_old, meta_new = pin_del
    ...
    class TestPinVersionsPrune:
        @pytest.mark.parametrize("n", [1, 2])
        def test_n(self, board, pin_prune, pin_name, n):
        ...
        def test_days(self, board, pin_prune, pin_name, days):
    ...
    class TestPinSearch:
        ...
        def test_name(self, board, df, search, matches):
            if board.fs.protocol == "rsc":
...
class TestBaseBoard:
    def test_pin_meta_cache_touch(self, tmp_path: Path, df):
...
@pytest.mark.fs_rsc
class TestBoardRsConnect:
    @pytest.fixture
    def board_short(self, fs_short):
        board_short = BoardRsConnect("", fs_short)
        return board_short

    def test_pin_write_rsc_full_name(self, df, board_short):
        board_short.pin_write(df, "susan/some_df", type="csv")

    def test_pin_search_admin_user(self, df, board_short, fs_admin):
    ...
    def test_pin_write_title_update(self, df, board_short):
    ...
    def test_pin_meta_is_full_name(self, df, board_short):
    ...
    def test_path_to_pin_safe(self, board_short):
        assert board_short.path_to_pin("me/some_pin") == "me/some_pin"

    def test_require_full_pin_name(self, board_short):
    ...
    def test_pin_read_public(self, df, board_short):
